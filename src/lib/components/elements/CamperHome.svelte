<script lang="ts">
  import caravan from '$lib/assets/caravan.png';
</script>

<section class="mb-20">
  <div class="mx-auto max-w-7xl px-4 lg:px-8">
    <div class="mb-10 text-center">
      <h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
        I Nostri <span class="text-secondary">Servizi</span>
      </h2>
      <p class="mt-4 text-lg text-gray-600">
        OMG, ditta creatrice del marchio VS, negli anni ha espanso la sua gamma d'intervento ed ora
        compie ristrutturazioni, riparazioni, migliorie e <b>costruzioni su misura</b> in un'ampia gamma
        di categorie, fra trasporti ausiliari e motorizzati.
      </p>
    </div>
    <div class="flex flex-col gap-8 md:flex-row md:items-stretch">
      <!-- Left features -->
      <div class="flex flex-1 flex-col justify-center gap-6">
        <!-- REVISIONI -->
        <a
          href="/articoli?activeFilter=Articolo"
          class="group hover:ring-secondary/50 relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
        >
          <div class="flex flex-col items-start">
            <div class="mb-4 flex items-center gap-4">
              <div
                class="bg-secondary/10 text-secondary group-hover:bg-secondary/20 flex size-12 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="size-8">
                  <path
                    d="M264 144L376 144C389.3 144 400 133.3 400 120C400 106.7 389.3 96 376 96L264 96C250.7 96 240 106.7 240 120C240 133.3 250.7 144 264 144zM264 176C235.8 176 212.4 155.1 208.6 128L192 128C174.3 128 160 142.3 160 160L160 512C160 529.7 174.3 544 192 544L448 544C465.7 544 480 529.7 480 512L480 160C480 142.3 465.7 128 448 128L431.4 128C427.5 155.1 404.2 176 376 176L264 176zM376 64C398.3 64 417.6 77.1 426.6 96L448 96C483.3 96 512 124.7 512 160L512 512C512 547.3 483.3 576 448 576L192 576C156.7 576 128 547.3 128 512L128 160C128 124.7 156.7 96 192 96L213.4 96C222.4 77.1 241.7 64 264 64L376 64zM320 288C311.2 288 304 280.8 304 272C304 263.2 311.2 256 320 256L416 256C424.8 256 432 263.2 432 272C432 280.8 424.8 288 416 288L320 288zM320 448L416 448C424.8 448 432 455.2 432 464C432 472.8 424.8 480 416 480L320 480C311.2 480 304 472.8 304 464C304 455.2 311.2 448 320 448zM264 464C264 477.3 253.3 488 240 488C226.7 488 216 477.3 216 464C216 450.7 226.7 440 240 440C253.3 440 264 450.7 264 464zM240 248C253.3 248 264 258.7 264 272C264 285.3 253.3 296 240 296C226.7 296 216 285.3 216 272C216 258.7 226.7 248 240 248zM336 368C336 359.2 343.2 352 352 352L416 352C424.8 352 432 359.2 432 368C432 376.8 424.8 384 416 384L352 384C343.2 384 336 376.8 336 368zM299.5 346.1L254.2 402.1C251.2 405.9 246.6 408 241.7 408C236.8 408 232.3 405.8 229.3 402L204.4 370.9C198.9 364 200 353.9 206.9 348.4C213.8 342.9 223.9 344 229.4 350.9L241.9 366.5L274.7 326C280.3 319.1 290.3 318.1 297.2 323.6C304.1 329.1 305.1 339.2 299.6 346.1z"
                  /></svg
                >
              </div>
              <h3
                class="text-secondary text-lg font-semibold transition-all group-hover:translate-x-1"
              >
                Revisioni
              </h3>
            </div>

            <div>
              <p class="text-gray-600">
                Revisioni e sistemazioni di qualsiasi problematica, sia meccanica che strutturale.
              </p>
            </div>
          </div></a
        >
        <!-- CARAVAN -->
        <a
          href="/caravan"
          class="group hover:ring-primary/50 relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
        >
          <div class="flex flex-col items-start">
            <div class="mb-4 flex items-center gap-4">
              <div
                class="bg-primary/10 text-primary group-hover:bg-primary/20 flex size-12 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="size-8">
                  <path
                    d="M480 128C497.7 128 512 142.3 512 160L512 416C512 433.7 497.7 448 480 448L302.4 448C287.8 419.5 258.2 400 224 400C189.8 400 160.2 419.5 145.6 448L96 448C78.3 448 64 433.7 64 416L64 160C64 142.3 78.3 128 96 128L480 128zM96 480L136.4 480C136.2 482.6 136 485.3 136 488C136 536.6 175.4 576 224 576C272.6 576 312 536.6 312 488C312 485.3 311.9 482.6 311.6 480L624 480C632.8 480 640 472.8 640 464C640 455.2 632.8 448 624 448L535.4 448C540.8 438.6 544 427.7 544 416L544 160C544 124.7 515.3 96 480 96L96 96C60.7 96 32 124.7 32 160L32 416C32 451.3 60.7 480 96 480zM336 192C327.2 192 320 199.2 320 208L320 336C320 344.8 327.2 352 336 352C344.8 352 352 344.8 352 336L352 208C352 199.2 344.8 192 336 192zM416 208L416 336C416 344.8 423.2 352 432 352C440.8 352 448 344.8 448 336L448 208C448 199.2 440.8 192 432 192C423.2 192 416 199.2 416 208zM144 192C135.2 192 128 199.2 128 208L128 336C128 344.8 135.2 352 144 352C152.8 352 160 344.8 160 336L160 208C160 199.2 152.8 192 144 192zM224 208L224 336C224 344.8 231.2 352 240 352C248.8 352 256 344.8 256 336L256 208C256 199.2 248.8 192 240 192C231.2 192 224 199.2 224 208zM224 432C254.9 432 280 457.1 280 488C280 518.9 254.9 544 224 544C193.1 544 168 518.9 168 488C168 457.1 193.1 432 224 432z"
                  />
                </svg>
              </div>
              <h3
                class="text-primary text-lg font-semibold transition-all group-hover:translate-x-1"
              >
                Caravan
              </h3>
            </div>
            <div>
              <p class="text-gray-600">
                Costruzione, assemblaggio e riparazione di Autocaravan, sia interni che esterni.
              </p>
            </div>
          </div>
        </a>
        <!-- CAMPER -->
        <a
          href="/caravan"
          class="group hover:ring-tertiary/50 relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
        >
          <div class="flex flex-col items-start">
            <div class="mb-4 flex items-center gap-4">
              <div
                class="bg-tertiary/10 text-tertiary group-hover:bg-tertiary/20 flex size-12 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="size-8">
                  <path
                    d="M256 104L256 112C256 120.8 248.8 128 240 128L144 128C99.8 128 64 163.8 64 208L64 377.5C64 388.4 67.7 399 74.5 407.5L111.6 453.9C127.5 431 154 416 184 416C224.3 416 258.2 443.1 268.7 480L384 480L384 208C384 199.2 391.2 192 400 192L576 192C576 156.7 547.3 128 512 128L400 128C391.2 128 384 120.8 384 112L384 104C384 99.6 380.4 96 376 96L264 96C259.6 96 256 99.6 256 104zM472 592C426.1 592 388.4 556.9 384.4 512L271.7 512C267.7 556.9 230 592 184.1 592C135.5 592 96.1 552.6 96.1 504C96.1 498.4 96.6 492.9 97.6 487.5L49.6 427.5C38.2 413.3 32 395.7 32 377.5L32 208C32 146.1 82.1 96 144 96L224.8 96C228.5 77.7 244.6 64 264 64L376 64C395.4 64 411.5 77.7 415.2 96L512 96C565 96 608 139 608 192L608 208C608 216.8 600.8 224 592 224L416 224L416 256L510.9 256C526.9 256 541.9 264 550.8 277.4L599.9 351.1C605.2 359 608 368.2 608 377.7L608 464C608 490.5 586.5 512 560 512L559.6 512C555.6 556.9 517.9 592 472 592zM556.7 480L560 480C568.8 480 576 472.8 576 464L576 384L416 384L416 436.1C431.2 423.5 450.7 416 472 416C512.3 416 546.2 443.1 556.7 480zM416 352L562.1 352L524.2 295.1C521.2 290.6 516.2 288 510.9 288L416 288L416 352zM184 560C214.9 560 240 534.9 240 504C240 473.1 214.9 448 184 448C153.1 448 128 473.1 128 504C128 534.9 153.1 560 184 560zM472 560C502.9 560 528 534.9 528 504C528 473.1 502.9 448 472 448C441.1 448 416 473.1 416 504C416 534.9 441.1 560 472 560zM168 192L280 192C302.1 192 320 209.9 320 232L320 280C320 302.1 302.1 320 280 320L168 320C145.9 320 128 302.1 128 280L128 232C128 209.9 145.9 192 168 192zM160 232L160 280C160 284.4 163.6 288 168 288L280 288C284.4 288 288 284.4 288 280L288 232C288 227.6 284.4 224 280 224L168 224C163.6 224 160 227.6 160 232z"
                  />
                </svg>
              </div>
              <h3
                class="text-tertiary text-lg font-semibold transition-all group-hover:translate-x-1"
              >
                Camper
              </h3>
            </div>
            <div>
              <p class="text-gray-600">
                Modifiche su richiesta, installazione nuove componenti, revisione delle
                apparechiature interne.
              </p>
            </div>
          </div>
        </a>
      </div>
      <!-- Center image -->
      <div class="flex flex-1 items-center justify-center">
        <img
          src={caravan}
          title="officine meccaniche giacomella"
          alt="Caravan Giacomella valsugana"
          class="w-full max-w-xs px-8 md:max-w-sm"
        />
      </div>
      <!-- Right features -->
      <div class="flex flex-1 flex-col justify-center gap-6">
        <!-- MOTORHOME -->
        <a
          href="/caravan"
          class="group hover:ring-secondary/50 relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
        >
          <div class="flex flex-col items-start">
            <div class="mb-4 flex items-center gap-4">
              <div
                class="bg-secondary/10 text-secondary group-hover:bg-secondary/20 flex size-12 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="size-8">
                  <path
                    d="M240 64L224 64L224 96L32 96L32 405.6L35.5 410L97.5 487.5C96.5 492.8 96 498.3 96 504C96 552.6 135.4 592 184 592C229.9 592 267.6 556.9 271.6 512L384.3 512C388.3 556.9 426 592 471.9 592C517.8 592 555.5 556.9 559.5 512L607.9 512L607.9 363.2L605.2 359.2L541.2 263.2L536.5 256.1L415.9 256.1L415.9 224.1L607.9 224.1L607.9 96.1L415.9 96.1L415.9 64.1L239.9 64.1zM519.4 288L562.1 352L416 352L416 288L519.4 288zM416 436.1L416 384L576 384L576 480L556.7 480C546.3 443.1 512.3 416 472 416C450.7 416 431.2 423.5 416 436.1zM384 480L268.7 480C258.3 443.1 224.3 416 184 416C154 416 127.5 431 111.6 453.9L64 394.4L64 128L256 128L256 96L384 96L384 128L576 128L576 192L384 192L384 480zM184 448C214.9 448 240 473.1 240 504C240 534.9 214.9 560 184 560C153.1 560 128 534.9 128 504C128 473.1 153.1 448 184 448zM472 448C502.9 448 528 473.1 528 504C528 534.9 502.9 560 472 560C441.1 560 416 534.9 416 504C416 473.1 441.1 448 472 448zM168 192C145.9 192 128 209.9 128 232L128 280C128 302.1 145.9 320 168 320L280 320C302.1 320 320 302.1 320 280L320 232C320 209.9 302.1 192 280 192L168 192zM160 232C160 227.6 163.6 224 168 224L280 224C284.4 224 288 227.6 288 232L288 280C288 284.4 284.4 288 280 288L168 288C163.6 288 160 284.4 160 280L160 232z"
                  /></svg
                >
              </div>
              <h3
                class="text-secondary text-lg font-semibold transition-all group-hover:translate-x-1"
              >
                Motorhome
              </h3>
            </div>
            <div>
              <p class="text-gray-600">O.M.G. è inoltre specializzata nella categoria Motorhome.</p>
            </div>
          </div>
        </a>
        <!-- MEZZI SPECIALI -->
        <a
          href="/articoli?activeFilter=Mezzi+Speciali"
          class="group hover:ring-primary/50 relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
        >
          <div class="flex flex-col items-start">
            <div class="mb-4 flex items-center gap-4">
              <div
                class="bg-primary/10 text-primary group-hover:bg-primary/20 flex size-12 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="size-8">
                  <path
                    d="M488.2 64.2C572.8 68.5 640 138.4 640 224L640 448L639.7 454.5C636.6 484.6 612.7 508.6 582.6 511.6L576.1 511.9L570.5 511.9C557.3 549.2 521.9 575.9 480.1 575.9C438.3 575.9 402.8 549.2 389.7 511.9L250.6 511.9C237.4 549.2 201.9 575.9 160.2 575.9C118.5 575.9 83 549.2 69.8 511.9L64.2 511.9L57.7 511.6C27.6 508.5 3.6 484.6 .6 454.5L0 448L0 160C0 108.6 40.3 66.7 91.1 64.1L96 64L480 64L488.2 64.2zM160 416C124.7 416 96 444.7 96 480C96 515.3 124.7 544 160 544C195.3 544 224 515.3 224 480C224 444.7 195.3 416 160 416zM480 416C444.7 416 416 444.7 416 480C416 515.3 444.7 544 480 544C515.3 544 544 515.3 544 480C544 444.7 515.3 416 480 416zM32 288L32 448C32 465.7 46.3 480 64 480C64 427 107 384 160 384C213 384 256 427 256 480L384 480C384 438.2 410.7 402.7 448 389.6L448 288L32 288zM480 384C533 384 576 427 576 480C593.7 480 608 465.7 608 448L608 224C608 153.3 550.7 96 480 96L480 384zM256 256L448 256L448 96L256 96L256 256zM96 96C60.7 96 32 124.7 32 160L32 256L224 256L224 96L96 96z"
                  /></svg
                >
              </div>
              <h3
                class="text-primary text-lg font-semibold transition-all group-hover:translate-x-1"
              >
                Mezzi Speciali
              </h3>
            </div>
            <div>
              <p class="text-gray-600">
                Preparazione e settaggio di mezzi speciali, Case Mobili, Autocaravan personalizzati
                ...
              </p>
            </div>
          </div>
        </a>
        <!-- ACCESSORI -->
        <a
          href="/articoli?activeFilter=Accessori"
          class="group hover:ring-tertiary/50 relative overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
        >
          <div class="flex flex-col items-start">
            <div class="mb-4 flex items-center gap-4">
              <div
                class="bg-tertiary/10 text-tertiary group-hover:bg-tertiary/20 flex size-12 items-center justify-center rounded-2xl transition-all duration-300 group-hover:scale-110"
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="size-8"
                  ><path
                    d="M57.4 102.7C52 108.1 51.2 116.5 55.4 122.9L134.6 241.7C140.5 250.6 150.5 255.9 161.2 255.9L233.4 255.9L309.7 332.2C285.1 367.5 288.5 416.4 320 447.9L446.2 574.1C464.9 592.8 495.3 592.8 514.1 574.1L574.2 514C592.9 495.3 592.9 464.9 574.2 446.1L448.1 320C416.6 288.5 367.7 285.1 332.4 309.7L256.1 233.4L256.1 161.2C256.1 150.5 250.8 140.5 241.9 134.6L123 55.3C116.7 51.1 108.2 51.9 102.8 57.3L57.4 102.7zM224.1 224L161.2 224L89.3 116.1L116.2 89.2L224.1 161.1L224.1 224zM551.5 491.3L491.4 551.4C485.2 557.6 475 557.6 468.8 551.4L342.7 425.4C319.9 402.5 319.9 365.5 342.7 342.7C365.5 319.9 402.6 319.9 425.4 342.7L551.5 468.7C557.7 474.9 557.7 485.1 551.5 491.3zM77.2 461.1C49.1 489.2 49.1 534.8 77.2 562.9C105.3 591 150.9 591 179 562.9L274 468C267.2 459.1 261.6 449.5 257.2 439.5L156.4 540.3C140.8 555.9 115.5 555.9 99.8 540.3C84.1 524.7 84.2 499.4 99.8 483.7L246.6 337L224 314.3L77.2 461.1zM288.7 113.9C295.7 123.4 300.5 134.4 302.7 146.1C327.6 115.5 365.6 96 408.1 96C423 96 437.3 98.4 450.7 102.8L388.8 164.7C385.8 167.7 384.1 171.8 384.1 176L384.1 240C384.1 241.9 384.4 243.8 385.1 245.5C402.8 245.6 420.5 249.1 437.2 256L464.2 256C468.4 256 472.5 254.3 475.5 251.3L537.4 189.4C541.8 202.8 544.2 217.1 544.2 232C544.2 264.5 532.8 294.3 513.8 317.7L536.5 340.4C561.3 311.1 576.2 273.3 576.2 232C576.2 203.8 569.3 177.3 557 153.9C554.6 149.4 550.3 146.3 545.2 145.5C540.1 144.7 535.1 146.4 531.5 150L457.6 223.9L416.2 223.9L416.2 182.5L490.1 108.6C493.7 105 495.4 99.9 494.6 94.9C493.8 89.9 490.7 85.5 486.2 83.1C462.8 70.9 436.3 64 408.1 64C361.4 64 319.1 83.1 288.7 113.9z"
                  ></path></svg
                >
              </div>
              <h3
                class="text-tertiary text-lg font-semibold transition-all group-hover:translate-x-1"
              >
                Accessori
              </h3>
            </div>
            <div>
              <p class="text-gray-600">
                Accessori e ricambi dei principali marchi specializzati: DIMATEC, ECOSAN, ALKO ...
              </p>
            </div>
          </div>
        </a>
      </div>
    </div>
  </div>
</section>
