...
>
  ...
  <div class="absolute inset-0 -z-10 overflow-hidden">
    <svg
      class="stroke-secondary/15 absolute top-0 left-[50%] h-full w-full -translate-x-1/2"
      aria-hidden="true"
    >
      <defs>
        <pattern
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-[1fr_auto] md:items-center">
      ...
      >
        {#each breadcrumbs as crumb, i}
          ...
          {:else}
            <span
              class="text-secondary line-clamp-1 truncate font-bold lg:max-w-52 xl:max-w-60 2xl:max-w-68"
              >{crumb.label}</span
            >
          {/if}
        {/each}
